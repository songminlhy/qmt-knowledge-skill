...
![](https://dict.thinktrader.net/assets/内置API_回测模型_实盘模型-a800c759.png)
...
1.  回测是遍历固定的历史数据：
    ...
    ![](https://dict.thinktrader.net/assets/内置API_操作_数据管理_下载全部-d3b5864c.png)
    
    -   首先需要下载历史行情，首次下载可以在界面左上角，点击`操作`，选择`数据管理`补充行情，选择回测的周期，如`日线`，所需的板块数据，如`沪深A股板块`，时间范围选择`全部`，下载完整历史行情
    
    ![](https://dict.thinktrader.net/assets/内置API_批量下载-c98af807.png)
...
![](https://dict.thinktrader.net/assets/内置API_基本信息_回测参数-eec55083.png)
...
> 🚫 **实盘风险提示（务必先读）**
>
> 「实盘模型」下调用 `passorder` 等下单函数会**真实发出委托到交易所**，可能造成实际资金损失。
> - 切换到实盘模式前，**请务必先在「模拟信号模式」或「模拟柜台」中完整验证策略逻辑**（包括信号生成、下单参数、撤单逻辑、风控阈值等）。
> - `quickTrade` 参数默认填 `0`（逐K线等待，安全）；`quickTrade=2` 在历史 Bar 上也会触发下单，回测/重放/重连场景极易产生意外委托，**实盘慎用**。
> - 股票交易受 2% 价格笼子限制，委托数量超过可用数量会产生废单；期货/期权存在杠杆风险。
> - 因使用本节代码造成的任何盈亏，由使用者自行承担全部责任。
...
![](https://dict.thinktrader.net/assets/内置API_三种机制对比-bc32690d.png)
...
![](https://dict.thinktrader.net/assets/内置API_不同机制匹配不同场景需求-6942d075.png)
...
因此，结合不同场景需求（回测或实盘），针对不同的机制（定时任务或���件驱动），我们分别给出回测与实盘的完整示例，复制到策略编辑器中即可使用。
